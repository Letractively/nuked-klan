    ...
    for($count = 0;$count < (6 * 7);$count++){
        ...
        if ($count < $firstdayarray['wday'] || $dayarray['mon'] != $month){
            ...
            if ($nuked['birthday'] != 'off'){
                $and = ($nuked['birthday'] == 'admin') ? ' niveau > 1' : '';
                $sql3 = mysql_query('SELECT pseudo, id, age FROM ' . USER_TABLE . $and);
                while (list($tnick, $tuid, $tage) = mysql_fetch_array($sql3)){
                    list ($tjour, $tmois, $tan) = explode ('/', $tage);

                    if ($tmois == $month && $tjour == $event_date && !empty($tnick) > 0){
                        $nb_birthday++;
                    }
                }
            }
            ...
            if ($nb_match > 0 || $nb_event > 0 || $nb_birthday > 0){
                ...
                if ($nb_birthday > 0){
                    $sql4 = mysql_query('SELECT id, pseudo, prenom, age FROM ' . USER_TABLE);
                    while (list($id_user, $pseudo, $prenom, $birthday) = mysql_fetch_array($sql4)){
