...
if ($visiteur >= $level_access && $level_access > -1){
    ...
    function detail($autor){
        ...
        $sql = mysql_query("SELECT U.id, U.icq, U.msn, U.aim, U.yim, U.email, U.url, U.age, U.sexe, U.prenom, U.interet, U.activite, U.ville, U.date, U.country, U.avatar, S.date FROM " . USER_TABLE . " AS U LEFT OUTER JOIN " . SESSIONS_TABLE . " AS S ON U.id = S.user_id WHERE U.pseudo = '" . $autor . "'");
        $test = mysql_num_rows($sql);

        if ($test > 0){
            list($id_user, $icq, $msn, $aim, $yim, $email, $url, $birthday, $sexe, $prenom, $interet, $activite, $ville, $date, $country, $photo, $last_used) = mysql_fetch_array($sql);
            list ($pays, $ext) = explode ('.', $country);
			...
			if ($interet) echo "<tr><td><b>&nbsp;&nbsp;ª " . _INTEREST . "&nbsp;:&nbsp;</b></td><td>" . $interet . "</td></tr>\n";
			if ($activite) echo "<tr><td><b>&nbsp;&nbsp;ª " . _ACTIVITY . "&nbsp;:&nbsp;</b></td><td>" . $activite . "</td></tr>\n";
			...
			if ($date) echo "<tr><td><b>&nbsp;&nbsp;ª " . _DATEUSER . "&nbsp;:&nbsp;</b></td><td>" . nkDate($date) . "</td></tr>";
			if ($last_used) echo "<tr><td><b>&nbsp;&nbsp;ª " . _LASTVISIT . "&nbsp;:&nbsp;</b></td><td>" . nkDate($last_used) . "</td></tr>";
